...
private _stateMachine = [_list, _skipNull] call CBA_statemachine_fnc_create;

{
    ...
    [_stateMachine, _onState, _onStateEntered, _onStateLeaving, _state] call CBA_statemachine_fnc_addState;
...
// We need to add the transitions in a second loop to make sure the states exist already
{
    private _state = configName _x;
    {
        ...
        if (_events isEqualTo []) then {
            [_stateMachine, _state, _targetState, _condition, _onTransition, _transition, _conditionFrequency] call CBA_statemachine_fnc_addTransition;
        } else {
            [_stateMachine, _state, _targetState, _events, _condition, _onTransition, _transition] call CBA_statemachine_fnc_addEventTransition;
        };
