...

{
    ...
    private _skipNull = _stateMachine getVariable "CBA_statemachine_skipNull";
    private _id = _stateMachine getVariable "CBA_statemachine_ID";
    private _tick = _stateMachine getVariable "CBA_statemachine_tick";
    ...
    if (_tick >= count _list) then {
        private _updateCode = _stateMachine getVariable "CBA_statemachine_updateCode";
        _tick = 0;
        if !(_updateCode isEqualTo {}) then {
            ...
            _stateMachine setVariable ["CBA_statemachine_list", _list];
        };
    };
    ...
    if !(_list isEqualTo []) then {
        _stateMachine setVariable ["CBA_statemachine_tick", _tick + 1];

        private _current = _list select _tick;
        private _thisState = _current getVariable ("CBA_statemachine_state" + str _id);
        private _lastCheck = _current getVariable [("CBA_statemachine_lastCheck" + str _id),CBA_missionTime];
        ...
        if (isNil "_thisState") then {
            ...
            _thisState = _stateMachine getVariable "CBA_statemachine_initialState";
            _current setVariable ["CBA_statemachine_state" + str _id, _thisState];
            _current setVariable ["CBA_statemachine_lastCheck" + str _id, CBA_missionTime];
        ...
        private _thisOrigin = _thisState;
        {
            ...
            if ((CBA_missionTime >= (_lastCheck + _conditionFrequency)) && {_current call _condition}) exitWith {
                ...
                _current setVariable ["CBA_statemachine_state" + str _id, _thisTarget];
                _current setVariable ["CBA_statemachine_lastCheck" + str _id, CBA_missionTime];
    ...
    (CBA_statemachine_performanceCounters select _id) pushBack _perfRunTime;
    #endif

} forEach CBA_statemachine_stateMachines;
